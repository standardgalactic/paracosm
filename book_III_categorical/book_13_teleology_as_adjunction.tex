...
\part{Applications and Higher Teleodynamics}

\chapter{Adjunction in Cognitive Architectures}

The perception-action cycle is modeled in this chapter as the categorical adjunction of Chapter 2 applied to a specifically cognitive instantiation of $\mathbf{RSVPField}$, with active inference read as the adjoint functor relationship between sensation and prediction: a system performing active inference is, on this account, continuously working to satisfy the natural isomorphism $\mathrm{Hom}(\mathcal{A}(X), Y) \cong \mathrm{Hom}(X, \mathcal{P}(Y))$ for its own sensory and motor configurations.
...
\part{Global Implications and Future Work}

\chapter{Teleological Cosmology}
...
\[
\mathrm{Hom}(\text{Expyrosis}(M), N) \cong \mathrm{Hom}(M, \text{Inflation}(N))
\]
...
\chapter{The Ethics of Anticipation}
...
\chapter*{Appendix C: Comparative Notes on Categorical Cognition and Bayesian Adjunctions}
\addcontentsline{toc}{chapter}{Appendix C: Comparative Notes on Categorical Cognition and Bayesian Adjunctions}

The active-inference framework referenced in Chapter 9 has its own existing categorical formulations relating Bayesian updating to adjoint functors between spaces of beliefs and observations; the present book's teleological adjunction $\mathcal{A} \dashv \mathcal{P}$ is related to, but not identical with, those formulations, differing specifically in that the present construction is built to preserve the thermodynamic structure of $\mathbf{RSVPField}$ throughout, a requirement standard Bayesian categorical treatments do not impose.

\chapter*{Appendix D: Reflective Note --- ``Purpose is the Adjoint of Time''}
\addcontentsline{toc}{chapter}{Appendix D: Reflective Note --- ``Purpose is the Adjoint of Time''}
