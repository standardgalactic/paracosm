...
\chapter{Cognitive Symmetry Breaking}
...
\chapter*{Appendix A: Algebraic Identities for Graded Commutators}
\addcontentsline{toc}{chapter}{Appendix A: Algebraic Identities for Graded Commutators}
...
\chapter*{Appendix B: Proof of Monotonicity of the Ethical Potential $\Psi$}
\addcontentsline{toc}{chapter}{Appendix B: Proof of Monotonicity of the Ethical Potential}

The monotonic decrease of $\Psi = \operatorname{Tr}(K^{\dagger}K)$, with $K = [\PhiField, \SEntropy]$, under the dynamics of Chapter 5 follows from expressing $\dot{\Psi}$ as twice the trace of $K^{\dagger}$ contracted with $\dot{K}$, then substituting the teleological operator's action on $\PhiField$ from Chapter 5 and observing that the resulting expression is a negative semidefinite quadratic form, so that $\dot{\Psi} \le 0$ along any trajectory generated by $\mathcal{T}$, with equality exactly at the fixed points characterized in Chapter 10. Because $\Psi$ is constructed as $\operatorname{Tr}(K^\dagger K)$ rather than $\operatorname{Tr}(K^2)$, this monotonic decrease is a decrease of a genuine nonnegative quantity toward zero, rather than a decrease of a quantity whose sign was never established, correcting an earlier, looser statement of this result.

\chapter*{Appendix C: Numerical Framework for Teleological Learning Operators}
\addcontentsline{toc}{chapter}{Appendix C: Numerical Framework for Teleological Learning Operators}

A minimal numerical implementation represents $\PhiField$ and $\SEntropy$ as low-dimensional matrices, as in the simulations of Book 1's Appendix B, and iterates the operator $\mathcal{T}$ directly, tracking the spectrum of the resulting sequence of matrices to confirm the persistence-versus-decay classification proposed in Chapter 5 and the convergence toward the fixed point of Chapter 10 under a range of initial asymmetries.

\chapter*{Appendix D: Philosophical Note: ``Every Asymmetry is an Invitation to Act''}
\addcontentsline{toc}{chapter}{Appendix D: Philosophical Note: ``Every Asymmetry is an Invitation to Act''}

The phrase heading this appendix restates, in compressed form, the relation between Chapter 2's infinitesimal asymmetry and Chapter 8's ethical potential. An asymmetry, once registered, is already a departure from the flat ethical cosmos of Chapter 1, and the minimization principle of Chapter 8 shows that such departures do not merely permit action but are themselves the gradient along which action, if it is to reduce $\Psi$, must proceed. This is offered as commentary on the formal results of this book rather than as an additional theorem, in the same spirit as the closing appendix of Book 1.
